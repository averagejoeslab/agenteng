...
Streaming is one real reason async exists in LLM applications. The agent we'll build later doesn't stream — it needs the full response to detect tool requests before acting — but you've now seen `async`/`await` in a context where its purpose is concrete.
