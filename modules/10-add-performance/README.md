...
- **Threading inside async tools.** When a tool blocks on CPU work (regex over a big tree, embedding inference), wrap the call in `asyncio.to_thread` so the event loop stays responsive.
